...
| `MateNet`            | The refutation of every defensive try, stepped to its real end     | **One** shared board plus a SAN list, never one preview per reply — a defender node in a net can have 24 legal replies, and 24 board instances on a 360px phone is not a design. Previous/next controls step the one board through `sequenceFens` to the actual checkmated position (#121); local state, not the URL. Marks no ply at the leaf itself, then the ply that reached each frame after, same as every other board |
